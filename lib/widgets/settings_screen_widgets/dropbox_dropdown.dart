...
import 'package:buildtest/helpers/configuration.dart';
import 'package:buildtest/helpers/config_loader.dart';
import 'package:buildtest/helpers/metadata_cacher.dart';
import 'package:buildtest/helpers/oauth_helper.dart';
import 'package:buildtest/providers/color_theme_provider.dart';
import 'package:buildtest/providers/update_provider.dart';
//import 'package:buildtest/screens/dropbox_signin_screen.dart';
import 'package:buildtest/widgets/settings_screen_widgets/connect_button.dart';
import 'package:buildtest/widgets/settings_screen_widgets/logout_button.dart';
import 'package:buildtest/widgets/settings_screen_widgets/refresh_button.dart';
...
class _DropboxDropdownState extends State<DropboxDropdown> {
  ...
  Future<bool> loadingToken() async {
    ...
    if (sp.containsKey('token')) {
      ...
      selected_calibre_lib_dir =
          sp.getString('selected_calibre_lib_path') ?? '';
      selected_lib_name = sp.getString('selected_calibre_lib_name') ?? '';
      noOfCalibreLibs = sp.getInt('noOfCalibreLibs') ?? 0;
...
//    print(code);
    ...
    Response response = await post(Uri(path: url), body: json);
    int statusCode = response.statusCode;
    String body = response.body;
    return response;
  }
